...
import { Application, GeometryType, MaterialFactory, MaterialType, ModelType } from '../../core';
import { Entity } from './Entity';
import { ObjectType } from '../../mixins';
import { ObjectTypeMesh } from '../extra/DragBehaviour';
...
export class Mesh extends Mixins(Entity) {
  ...
  protected async instantiate() {
    ...
    mesh.name = this.name;
    await this.app.mesh.set(this.name, mesh)
    return mesh;
  ...
  private async createMeshFromGeomAndMat() {
    ...
    const [material, geometry] = await Promise.all<any, any>([promMat, promGeom]);
    const mesh = new THREE.Mesh(geometry as GeometryType, material);
    if (this.factory) {
      const texture = await this.factory(this.app);
      mesh.add(texture)
    }
    // mesh)
    return mesh
