import Vue from 'vue'
import { FunctionalComponentOptions, VNode, VNodeDirective } from 'vue/types'
...
export function filterObjectOnKeys<T, K extends keyof T>(obj: T, keystones: K[]): { [N in K]: T[N] } {
  const filtered = {} as { [N in K]: T[N] }
  for (const key of keystones) {
...
export function arrayDiff(a: any[], b: any[]): any[] {
  const diff: any[] = []
  for (const bi of b) {
    if (a.indexOf(bi) < 0) diff.push(bi)
  }
  return diff
}
